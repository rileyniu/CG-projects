...
public class Quat extends AbstractList<Float> implements Cloneable {
    ...
    public static Quat slerp(Quat i1, Quat i2, float t)
    {
    	...
    	Quat q = null;
    	double cosine = i1.x * i2.x + i1.y * i2.y + i1.z * i2.z + i1.w * i2.w;
    	if (cosine < 0)
        {
          cosine = -cosine;
        }
    	...
    	if (angle < 0.01f) {
    		...
    		q = i1.clone().scale(1-t).add(i2.clone().scale(t));
    		q = q.normalize();
    	}else {
    		//slerp
    		float t1 = (float)(Math.sin(angle-t*angle)/Math.sin(angle));
    		float t2 =  (float)(Math.sin(t*angle)/Math.sin(angle));
    		q = i1.clone().scale(t1).add(i2.clone().scale(t2));
    		q = q.normalize();
    	}
    	return q;
    }
